...
for(i in names(comparisons)) {
  ...
  if(length(c2)==0|length(c1)==0){}else{
    ...
    pdf(paste0(base_img_dir,"padj_density_",condition,"_",i,".pdf"))
    plot(density(resContrast_sig$padj), 
         main = paste(condition,i,"padj density"),
         xlab = "padj")
    dev.off()
    
    pdf(paste0(base_img_dir,"padj_log_density_",condition,"_",i,".pdf"))
    plot(density(-log(resContrast_sig$padj)), 
         main = paste(condition,i,"padj density"),
         xlab = "-log(padj)")
    dev.off()
    
    pdf(paste0(base_img_dir,"p-value_density_",condition,"_",i,".pdf"))
    plot(density(resContrast_sig$pvalue), 
         main = paste(condition,i,"p-value density"),
         xlab = "padj")
    dev.off()
    
    pdf(paste0(base_img_dir,"p-value_log_density_",condition,"_",i,".pdf"))
    ...
    if(dim(res)[1] !=0) {
      ...
      pdf(paste0(img_dir,"volcano_plot_",condition,"_",i,"_",dname,".pdf"), width = 6, height = 6, units = 'in', res = 300)
      ...
      pdf(paste0(img_dir,"volcano_plot_",condition,"_",i,"_",dname,"_annot_synonyms.pdf"), width = 6, height = 6, units = 'in', res = 300)
      ...
      pdf(paste0(img_dir,"volcano_plot_",condition,"_",i,"_",dname,"_annot_genes.pdf"), width = 6, height = 6, units = 'in', res = 300,family="ArialMT")
      plot(res_vp$log2FoldChange,-log(res_vp$padj),log="y",col="gray",xlab=paste0("log2(",c1,"/",c2,")"),ylab="-log(p-value)",pch=20,main=i,cex=1.3,cex.axis=1.3,cex.lab=1.3)
      for(id in select_ID) {
      ...
      for (r in regulation){
        if(nrow(res[res$REGULATION==r,]) >2) {
          ...
          pdf(paste(img_dir,"heatmap_",condition,"_",i,"_",dname,"_",r,".pdf",sep=""), width = 8, height = 8, units = 'in', res = 300)
...
for (r in regulation){
  ...
  pdf(paste(img_dir,"heatmap_",condition,"_",dname,"_",r,".pdf",sep=""), width = 8, height = 8, units = 'in', res = 300)
  par(mar=c(8.1, 4.1, 4.1, 4.1), xpd=TRUE)
  heatmap.2(as.matrix(log2(data+1)),
            Rowv=as.dendrogram(hcGenes), Colv=NULL,
            scale="row", labRow="",trace="none", col = hmcol,
...
pdf(paste0(img_dir,"barplot_autogamy_proportion.pdf"),family="ArialMT")
...
pdf(paste0(img_dir,"barplot_autogamy_cluster_proportion.pdf"))
...
pdf(paste0(img_dir,"barplot_proportion_genes_with_IES.pdf"))
