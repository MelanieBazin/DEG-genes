...
rm(Tous_les_genes,Genes_codant,
   UP_PGM, UP_KU80c, UP_XRCC4, UP_EZL1, DOWN_CTIP,
   UP_PGM_KU80c, UP_PKX, UP_PKXE, 
   UP_PKX_DOWN_CTIP, UP_DOWN ,
   DE_autogamie, Pic_intermediaire,
...
for (type in c("fuzznuc", "fimo", "FIMO2")){
  ...
  for (f in files){
    ...
    # Overture des fichier motifs
    prom_with_motif = read.table(paste0(path_motif,dossier,f), header=F, sep="\t")
    colnames(prom_with_motif)=colnamesgff3
    prom_with_motif$START = prom_with_motif$START-150
    prom_with_motif$END = prom_with_motif$END-150
    # Filtrer sur les motif dans la région d'interet
    prom_with_motif = prom_with_motif[prom_with_motif$START <= -50 & prom_with_motif$START >= -80,]
    
    infogenes_motif = merge(infogenes,prom_with_motif, by = "ID" )
    write.table(infogenes_motif, paste0(save_path,sub(".gff","",sub(".gff3","",f)),"_INFO_filtre.tab"), sep = "\t", row.names = F)
    ...
    for(filtre in names(FILTRES)){
      ...
      filter_motif2 = prom_with_motif[is.element(prom_with_motif$ID, selection),]
    ...
    write.table(effectif_tab, paste0(save_path,sub(".gff","",sub(".gff3","",f)),"_Motif_avec_duplicat.tab"), sep = "\t")
    
    pdf(paste0(save_path,sub(".gff","",sub(".gff3","",f)),"_Motif_graphs.pdf"))
    ...
    if (sum(effectif_tab_Noduplicat$`Motif_+/-`)>0){
      write.table(effectif_tab_Noduplicat, paste0(save_path,sub(".gff","",sub(".gff3","",f)),"_Motif_sans_duplicat.tab"), sep = "\t")
...
# Cf 6_DistributionMotif_motif_restreint