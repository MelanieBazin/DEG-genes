...
if (condition == "CTIP") {
  label = c("Veg", "T0","", "T5/5.5","", "T10", "12.5","","","T20","","T25", "","T30") 
  x_ctrl = c(1,2,4,6,10,14)
  x_rnai = c(1,2,4,7,12)
...
if (condition == "XRCC4"){
  label = c("Veg", " ","T2", "","T7", "", "","","","","T22","", "","","T32")
  x_ctrl = c(1,3,5,11,15)
  x_rnai = c(1,3,5,11,15)
...
####### Graphique pour une sélection de gènes #####
...
for( i in 1:length(selection)){
  ...
  select_annotation = c(select_annotation, grep(selection[i],annotation$SYNONYMS))
  }
select_ctrl = control[select_ctrl,]
select_rnai = rnai[select_rnai,]
select_annotation = annotation[select_annotation,]
#write.table(select_annotation, paste0("./Analyse_petit/","/Selection_",condition,"_",type,".tab"), sep = "\t", na ="",row.names = F)

pdf(paste0("./Analyse_petit/", condition,"_",type, "_controlvsRNAi_selection",".pdf"))
for (i in 1:nrow(select_ctrl)){
  plot(NULL, 
       xlim = c(1,length(label)), 
       ylim=c(0,ceiling(max(select_ctrl[i,-c(1:5)],select_rnai[i,-c(1:5)]))),
       axes=F,
       ylab= type,
       xlab="Temps autogamie",
       main = paste0(select_ctrl$ID[i]," - ", select_ctrl$SYNONYMS[i]))
  axis(1,at=1:length(label),labels=label,las=2)
  axis(2)
  legend("topright",legend=c("control",paste0("RNAi ", condition)),col=c(1,2),lwd=2)
  
  lines(x_ctrl, select_ctrl[i,-c(1:5)],col=1,lwd=2)
  lines(x_rnai, select_rnai[i,-c(1:5)],col=2,,lwd=2)
}

dev.off()


####### Mise en forme des donnée pour DESeq2 ##############
...
#infodata = cbind(infodata, c(colnames(control)[6:ncol(control)],colnames(rnai)[6:ncol(rnai)]))
