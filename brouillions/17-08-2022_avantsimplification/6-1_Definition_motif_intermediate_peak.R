...
save_path = paste0("./Analyse/",file_name, "/", condition, "/Motif/From_",debut, "_IN_MAC",IES,"/intermediate/")
...
path = paste0(save_path, "Promotor/")
dir.create(path,recursive=T,showWarnings=F)

prom_int = promoteur[which(is.element(names(promoteur),AUTOGAMY$inter_peak))]
write.fasta(sequences = prom_int, names = names(prom_int), file.out = paste0(path, "Prom_inter.fa") )
